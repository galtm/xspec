...
<p:library xmlns:p="http://www.w3.org/ns/xproc" xmlns:s="x-urn:test:xproc:steplibrary"
    ...
    <!-- This pipeline shows an example where x:input-position()
        would determines the wrong position for an input because compute-position.xsl
        has no logic to handle p:input/@use-when. -->
