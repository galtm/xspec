   ...
   <xsl:function name="x:step-call-text" as="text()">
      ...
      <xsl:for-each select="$call">
         ...
         <xsl:value-of>
            ...
            <xsl:iterate select="($declared-inputs, x:option[1])">
               ...
               <xsl:choose>
                  <xsl:when test="self::p:input">
                     <xsl:variable name="x-input" as="element(x:input)*"
                        select="$call/x:input[@name eq string($argument-to-construct/@port)]"/>
                     <xsl:choose>
                                    ...
                                    <xsl:call-template name="x:prefix-diag-message">
                                       ...
                                       <xsl:with-param name="message">
                                          <xsl:text expand-text="yes">Declaration of port {
                                             $x-input/@name} is under @use-when, which XSpec assumes is true.</xsl:text>
                                       </xsl:with-param>
                                    </xsl:call-template>
                                 </xsl:message>
                              </xsl:for-each>
