...
<xsl:stylesheet xmlns="http://www.w3.org/1999/XSL/TransformAlias"
   ...
   <xsl:template name="x:compile-scenario" as="element(xsl:template)+">
      ...
      <xsl:if test="$call/@template">
         <xsl:message terminate="yes">
            <xsl:call-template name="x:prefix-diag-message">
               <xsl:with-param name="message" as="xs:string">
                  <xsl:text>Can't call a template in a test suite for XProc</xsl:text>
               </xsl:with-param>
            </xsl:call-template>
         </xsl:message>
      </xsl:if>
      <xsl:if test="$call/@function">
         <xsl:message terminate="yes">
            <xsl:call-template name="x:prefix-diag-message">
               <xsl:with-param name="message" as="xs:string">
                  <xsl:text>Can't call a function in a test suite for XProc</xsl:text>
               </xsl:with-param>
            </xsl:call-template>
         </xsl:message>
      </xsl:if>
               ...
               <variable name="{x:known-UQName('x:result')}" as="item()*">
                  ...
                  <xsl:for-each select="($call)[1]/(x:input | x:option)">
                     <xsl:apply-templates select="." mode="x:declare-variable">
                        <xsl:with-param name="comment" select="concat(@name, ' ', local-name())" />
                     </xsl:apply-templates>
                  </xsl:for-each>
                        ...
                        <xsl:call-template name="x:enter-sut">
                           <xsl:with-param name="instruction" as="element(xsl:sequence)">
                              <sequence>
                                 ...
                                 <xsl:attribute name="select" select="x:step-call-text($call)" />
                              </sequence>
                           </xsl:with-param>
                        </xsl:call-template>
                     </xsl:when>
   ...
   <xsl:include href="compute-position.xsl"/>
   <xsl:function name="x:step-call-text" as="text()">
      ...
      <xsl:for-each select="$call">
         <xsl:variable name="step-uqname" as="xs:string"
            select="x:UQName-from-EQName-ignoring-default-ns(@step, .)"/>
         <xsl:variable name="inputs-sorted-as-in-step" as="element(x:input)*">
            <xsl:perform-sort select="x:input">
               <xsl:sort select="x:input-position(.)"/>
            </xsl:perform-sort>
         </xsl:variable>
         <xsl:variable name="option-map-text" as="text()">
            <xsl:call-template name="create-option-map-text"/>
         </xsl:variable>

         <xsl:value-of>
            <xsl:text expand-text="yes">{$step-uqname}(</xsl:text>
            <xsl:iterate select="($inputs-sorted-as-in-step, x:option[1])">
               <xsl:param name="separator" as="xs:string" select="''"/>
               <xsl:sequence select="$separator"/>
               <xsl:choose>
                  <xsl:when test="self::x:input">
                     <xsl:text expand-text="yes">${x:variable-UQName(.)}</xsl:text>
                  </xsl:when>
                  <xsl:otherwise>
                     <xsl:sequence select="$option-map-text"/>
                  </xsl:otherwise>
               </xsl:choose>
               <xsl:next-iteration>
   ...
   <xsl:template name="create-option-map-text" as="text()">
      ...
      <xsl:value-of>
         <xsl:iterate select="x:option">
            ...
            <xsl:value-of expand-text="yes">{$start-or-between}'{@name}': ${x:variable-UQName(.)}</xsl:value-of>
            <xsl:next-iteration>
