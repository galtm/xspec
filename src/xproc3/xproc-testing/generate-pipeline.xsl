...
<xsl:stylesheet version="3.0" xmlns:p="http://www.w3.org/ns/xproc" xmlns:x="http://www.jenitennison.com/xslt/xspec"
    ...
    <xsl:param name="xproc-uri" as="xs:anyURI" required="yes"/>
    ...
    <xsl:template name="generate-pipeline" as="element(p:declare-step)">
        <p:declare-step version="3.1" xmlns:x="http://www.jenitennison.com/xslt/xspec">
            <xsl:call-template name="import-tested-pipeline"/>
    ...
    <xsl:template name="import-tested-pipeline" as="node()+">
        <xsl:comment>Import the pipeline mentioned in x:description/@xproc</xsl:comment>
        <p:import href="{$xproc-uri}"/>
    </xsl:template>
    ...
    <xsl:template name="declare-substep" as="node()+">
        ...
        <p:declare-step name="test-in-scope-step-functions" type="x:test-in-scope-step-functions">
            <xsl:attribute name="xml:base" select="resolve-uri($xspec-home || '/src/xproc3/')"/>
