...
:xslt-with-pipeline
    rem # Set XPROC_FILE variable to x:description/@xproc value
    FOR /F "delims=" %%V IN ('java -cp "%SAXON_CP%" net.sf.saxon.Transform -s:"%XSPEC%" -xsl:"%XSPEC_HOME%\src\cli\get-xproc-uri.xsl" -it %CATALOG%') DO SET "XPROC_FILE=%%V"
    java ^
        -Dxspec.coverage.ignore="%TEST_DIR%" ^
        -Dxspec.coverage.xml="%COVERAGE_XML%" ^
        -Dxspec.home="%XSPEC_HOME%" ^
        -Dxspec.xspecfile="%XSPEC%" ^
        -Dcom.xmlcalabash.pipelines="%XPROC_FILE%" ^
